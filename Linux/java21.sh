...
<<COMMENT
  ...
  Summary:
  The following code will install Java (OpenJDK 21), Apache Maven, and Gradle.
...
# --- Configuration ---
MAVEN_VERSION="3.9.6" # Current stable version at the time of this script
GRADLE_VERSION="8.7" # Current stable version at the time of this script
...
echo "--- Java 21, Maven, and Gradle Installation Script ---"
...
case "$PACKAGE_MANAGER" in
    apt)
        # For Ubuntu/Debian, the package name for OpenJDK 21 is usually 'openjdk-21-jdk'
        $INSTALL_CMD openjdk-21-jdk
        ;;
    yum_dnf)
        # On RHEL/CentOS/Fedora, it's typically 'java-21-openjdk-devel' for JDK
        $INSTALL_CMD java-21-openjdk-devel
        ;;
    pacman)
        # On Arch, it's usually 'jdk21-openjdk'
        $INSTALL_CMD jdk21-openjdk
        ;;
esac

if [ $? -ne 0 ]; then
    echo "Java (OpenJDK 21) installation failed. Exiting."
    exit 1
fi
...
if [ -z "$JAVA_HOME_PATH" ]; then
    echo "Warning: Could not determine JAVA_HOME automatically. Attempting common path for Java 21."
    # Fallback to common path if auto-detection fails, but warn user
    case "$OS_ID" in
        ubuntu|debian) JAVA_HOME_PATH="/usr/lib/jvm/java-21-openjdk-amd64" ;; # Typical path for Ubuntu 22.04+
        centos|rhel|fedora) JAVA_HOME_PATH="/usr/lib/jvm/java-21-openjdk" ;;
        arch) JAVA_HOME_PATH="/usr/lib/jvm/java-21-openjdk" ;;
        *) JAVA_HOME_PATH="/usr/lib/jvm/java-21-openjdk" ;; # Generic fallback
    esac
    echo "Using fallback JAVA_HOME: $JAVA_HOME_PATH"
fi
